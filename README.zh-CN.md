...
agent-forge 是一个基于 LIAS（Lightweight Industrial Agent Specification）规范的 Agent 工程化工具链，提供验证、评审、优化、生成一站式 CLI 支持。
...
将旧的 `agent.yaml` 格式升级为现代的 `AGENT.md` Markdown 格式：
